    ...
    else
    {
       await stocks.forEach(item => {
            ...
            console.log(data);
             ...
             db.collection('stocks').doc(`${item.ticker}`)
                  .get()
                  .then(doc=>{
                      if(doc.exists)
                      {
                        return  db.collection('stocks').doc(`${item.ticker}`).update({
                      ...
                      {
                        return db.collection('stocks').doc(`${item.ticker}`).set({Investors:[data]});
                      }
                  })
                  .then(res=>{
                      s.add(item.ticker);
...
exports.deletestocks = async (req,res)=>{
    try{
     ...
     const email=JSON.stringify(req.headers.email);
          ...
          await db.collection('users').doc(`${JSON.parse(email)}`).get()
                 .then(doc=>{
                                ...
                                if(element.ticker==item.ticker)
                                   {
                                    ...
                                    db.collection('users').doc(`${JSON.parse(email)}`).update({
                                        ...
                                        db.collection('stocks').doc(`${item.ticker}`).update({
                                            Investors:admin.firestore.FieldValue.arrayRemove(investor_data)
                                        }).then(re=>{
                                ...
                                if(element.ticker==item.ticker)
                                   {
                                    ...
                                    db.collection('users').doc(`${JSON.parse(email)}`).update({
                                        ...
                                        db.collection('stocks').doc(`${item.ticker}`).update({
                                            ...
                                            console.log("Changed in stock as well");
                                            db.collection('users').doc(`${JSON.parse(email)}`).update({
                                              stock_data:admin.firestore.FieldValue.arrayUnion(new_data)  
                                            })
                                        })
                                        .then(re=>{
                                            console.log("Changed in users");
                                            db.collection('stocks').doc(`${item.ticker}`).update({
                                                Investors:admin.firestore.FieldValue.arrayUnion(newuser)
                                            })
                                        })
                                        .then(re=>{
